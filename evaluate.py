...
For labeled alerts it prints:
  outcomes     the errors that matter on-call, next to the static
               configured-severity baseline (routing without Jev)
...
def compute_report(results, alerts):
    ...
    baseline = {a["id"]: triage.route_standalone(a, None, policy, "static baseline")
                for a in alerts}
...
def print_report(results, alerts):
    ...
    print(f"  {'':<44}{'jev':>6}{'static':>8}")
    for key, desc in OUTCOMES:
        other = "-" if key in NOT_APPLICABLE_TO_BASELINE else theirs[key]
        print(f"  {desc:<44}{ours[key]:>6}{other:>8}")
    print(f"  {'pages sent':<44}{ours['pages']:>6}{theirs['pages']:>8}")
    print(f"  {'reviews sent':<44}{ours['reviews']:>6}{theirs['reviews']:>8}")
