...
class Config:
    ...
    review_store: str | None = None  # JSONL path; set, pending reviews survive a restart
...
def load_config(path=None):
    ...
    _check_keys(raw, ["jev", "policy", "teams", "topology", "shadow", "server", "reviews"], path)
    ...
    reviews_cfg = raw.get("reviews", {})
    _check_keys(reviews_cfg, ["store"], f"{path} [reviews]")
    if "store" in reviews_cfg:
        if not isinstance(reviews_cfg["store"], str) or not reviews_cfg["store"].strip():
            raise ConfigError(f"{path} [reviews]: store must be a file path")
        config.review_store = reviews_cfg["store"]
    return config
...
def judge_all(alerts, candidates, api_key, model=MODEL, timeout_s=2.0, retries=1,
    ...
    def collect(answers):
        for aid, judgment, error, call in answers:
    ...
    if workers <= 1 or sys.platform == "emscripten":  # no threads in the browser demo
        collect(map(one, todo))
    else:
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
            collect(pool.map(one, todo))
    return judgments, errors, calls
