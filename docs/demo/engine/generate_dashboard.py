...
.ev-card.triaging .ev-arrow,
.ev-card.triaging .ev-decision,
.ev-card.triaging .ev-latency,
.ev-card.triaging .prob-bar,
...
@keyframes triageBlink { from { opacity: .4; } to { opacity: 1; } }
.ev-card .ev-arrow, .ev-card .ev-decision, .ev-card .ev-latency,
.ev-card .prob-bar, .ev-card .ev-bottom, .ev-card .ev-review-banner,
.ev-card .ev-kind-badge { transition: opacity .25s ease-out, visibility .25s; }
.ev-card.triaged .ev-kind-badge { animation: revealPop .3s ease-out; }
.ev-card.triaged .prob-bar { animation: revealSlide .4s ease-out; }
@keyframes revealPop { from { opacity: 0; transform: scale(.8); } to { opacity: 1; transform: scale(1); } }
@keyframes revealSlide { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: translateY(0); } }
...
(function () {
  ...
  function autoNext() {
    ...
    revealTimer = setTimeout(function () {
      card.classList.remove("triaging");
      card.classList.add("triaged");
      if (pane) { pane.style.transition = "opacity .25s"; pane.style.opacity = "1"; }
  ...
  function startAutoRotate() {
    ...
    autoRunning = true;
    updatePlayBtn();
    autoTimer = setTimeout(autoNext, 150);
  }
  function stopAutoRotate() {
    ...
    clearTriage();
    var pane = field("insp-analysis");
    if (pane) { pane.style.transition = ""; pane.style.opacity = ""; }
    updatePlayBtn();
  }
  ...
  function openAlert(id) {
    var card = document.querySelector('.ev-card[data-id="' + CSS.escape(id) + '"]');
  ...
  document.addEventListener("click", function (e) {
    var a = e.target.closest('a[href^="#alert-"]');
