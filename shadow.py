...
def validate_label(body, teams):
    """Check a label posted to /label/<id>. Returns the clean label or raises
    ValueError with a message the caller can show."""
...
class ShadowLog:
    ...
    def start(self, config):
        self._append({"type": "start", "at": _now(), "model": config.model,
    ...
    def decision(self, alert, record):
        ...
        self._append({"type": "decision", "at": _now(), "id": alert["id"], "alert": alert,
    ...
    def review(self, alert_id, outcome, by=None):
        ...
        self._append({"type": "review", "at": _now(), "id": alert_id, "outcome": outcome,
                      "by": by})

    def label(self, alert_id, label, by=None):
        self._append({"type": "label", "at": _now(), "id": alert_id, "label": label, "by": by})
